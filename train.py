...
class Trainer:
    ...
    def create_data_loader(self, config):
        ...
        val_x_img, val_x_meta, val_y_rgb, val_y_meta = np.flip(x_img), np.flip(x_meta), np.flip(y_rgb), np.flip(y_meta)
        self.train_dst = MetaDataLoader(x_img, x_meta, y_rgb, y_meta)
        self.val_dst = MetaDataLoader(val_x_img, val_x_meta, val_y_rgb, val_y_meta)
    ...
    def validate(self, model, global_step, epoch, device):
        ...
        model.eval()
        with torch.no_grad():
            for i, (val_x_img, val_x_meta, val_y_rgb, val_y_meta) in tqdm(enumerate(self.val_loader)):
                print("validating .....")
                val_x_img, val_x_meta =val_x_img.permute(0, 3, 1, 2), val_x_meta.permute(0, 3, 1, 2)
                val_y_rgb, val_y_meta = val_y_rgb.permute(0, 3, 1, 2), val_y_meta.permute(0, 3, 1, 2)

                val_x_img = val_x_img.to(device=device)
                val_x_meta = val_x_meta.to(device=device)
                val_y_rgb = val_y_rgb.to(device=device)
                val_y_meta = val_y_meta.to(device=device)
                 
                pred_rgb_, pred_meta_ = model(val_x_img, val_x_meta)
                val_rgb_loss = self.criterion.rgb_loss(pred_rgb_, val_y_rgb)
                val_shape_loss = self.criterion.shape_loss(pred_meta_, val_y_meta)
    ...
    def train(self, config, device, weight_path=None):
        ...
        for epoch in range(1, config.epochs+1):
            ...
            with tqdm(total=int(len(self.train_dst)/config.train_batch), desc=f'Epoch {epoch}/{config.epochs}') as pbar:
                for x_img, x_meta, y_rgb, y_meta in self.train_loader:
                    x_img, x_meta = x_img.permute(0, 3, 1, 2), x_meta.permute(0, 3, 1, 2)
                    y_rgb, y_meta = y_rgb.permute(0, 3, 1, 2), y_meta.permute(0, 3, 1, 2)
                    x_img = x_img.to(device=device)
                    x_meta = x_meta.to(device=device)
                    y_rgb = y_rgb.to(device=device)
                    y_meta = y_meta.to(device=device)

                    pred_rgb, pred_meta = model(x_img, x_meta)
                    rgb_loss = self.criterion.rgb_loss(pred_rgb, y_rgb)
                    shape_loss = self.criterion.shape_loss(pred_meta, y_meta) 
                    ...
                    if global_step % 10 == 0:
                        ...
                        self.tfwriter.add_scalar('train/rgb_loss', rgb/10, global_step)
                        self.tfwriter.add_scalar('train/shape_loss', shape/10, global_step)
