...
class Trainer:
    ...
    def create_data_loader(self, config):
        ...
        self.train_dst = MetaDataLoader(config.x_img, config.y_rgb, val=None)
        self.val_dst = MetaDataLoader(config.x_img, config.y_rgb, val=True)
    ...
    def validate(self, model, global_step, epoch, device):
        ...
        with torch.no_grad():
            for i, batch in tqdm(enumerate(self.val_loader)):
                ...
                # val loss update
                val_rgb_loss = self.criterion(val_y_rgb, pred_rgb_)
            ...
            print("Epoch %d, Itrs %d, valid_Loss=%f, valid_rgb=%f" % (epoch, global_step, interval_valloss/nums, val_rgb/nums))
        ...
        for epoch in range(1, config.epochs+1):
            ...
            with tqdm(total=int(len(self.train_dst)/config.train_batch), desc=f'Epoch {epoch}/{config.epochs}') as pbar:
                for batch in self.train_loader:
                    x_img = batch['image'].to(device=device, dtype=torch.float32)
                    y_rgb = batch['mask1'].to(device=device, dtype=torch.float32)
                    # predict
                    pred_rgb = model(x_img)
                    # loss update
                    rgb_loss = self.criterion(y_rgb, pred_rgb)
                    ...
                    if global_step % 10 == 0:
                        ...
                        print("Epoch %d, Itrs %d, Loss=%f, rgb_loss=%f" %
                            (epoch, global_step, interval_loss/10, rgb_loss.item()/10))
