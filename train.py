...
class Trainer:
    ...
    def create_data_loader(self, config):
        ...
        self.train_dst = MetaDataLoader(config.x_img, config.y_meta, val=None)
        self.val_dst = MetaDataLoader(config.x_img, config.y_meta, val=True)
    ...
    def validate(self, model, global_step, epoch, device):
        ...
        with torch.no_grad():
            for i, batch in tqdm(enumerate(self.val_loader)):
                ...
                val_y_meta = batch['mask'].to(device=device, dtype=torch.float32)
                # predict valid
                pred_meta = model(val_x_img)

                # val loss update
                val_loss = dice_loss(val_y_meta, pred_meta)
        ...
        for epoch in range(1, config.epochs+1):
            ...
            with tqdm(total=int(len(self.train_dst)/config.train_batch), desc=f'Epoch {epoch}/{config.epochs}') as pbar:
                for batch in self.train_loader:
                    ...
                    y_meta = batch['mask'].to(device=device, dtype=torch.float32)
                    # predict
                    pred_meta = model(x_img)
                    # loss update
                    loss = dice_loss(y_meta, pred_meta)
